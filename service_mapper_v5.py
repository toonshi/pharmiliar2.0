...
class ServiceMapper:
    ...
    def find_matching_services(self, medical_term: str) -> List[Dict]:
        ...
        for category in mapping["categories"]:
            placeholders = ','.join('?' * len(mapping["terms"]))
            exclude_terms = mapping.get("exclude", [])
            exclude_clause = ""
            if exclude_terms:
                exclude_placeholders = ','.join('?' * len(exclude_terms))
                exclude_clause = f"AND NOT (LOWER(description) LIKE ANY (SELECT '%' || LOWER(term) || '%' FROM (VALUES {exclude_placeholders})))"
            ...
            query = f"""
                ...
                WHERE category = ?
                AND (
                    {' OR '.join(['LOWER(description) LIKE ?' for _ in mapping["terms"]])}
                )
                {exclude_clause}
                AND base_price > 0
            ...
            if exclude_terms:
                params.extend([term.lower() for term in exclude_terms])
