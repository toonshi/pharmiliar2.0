...
class CostPredictor:
    ...
    def estimate_costs(self, analysis: Dict) -> Dict:
        ...
        for test in analysis["diagnostic_tests"]:
            cursor.execute("""
                SELECT description, base_price, max_price, category 
                FROM services 
                WHERE LOWER(description) LIKE ? 
                AND category IN ('LAB', 'LABORATORY', 'RADIOLOGY', 'DIAGNOSTICS')
            """, (f"%{test.lower()}%",))
            ...
            if results:
                cost_estimates["diagnostic_tests"].append({
                    ...
                    "max_cost": results[0][2] or results[0][1],
                    "description": results[0][0],
                    "category": results[0][3]
                })
                cost_estimates["total_min"] += results[0][1]
                cost_estimates["total_max"] += results[0][2] or results[0][1]
        ...
        for treatment in analysis["treatments"]:
            cursor.execute("""
                ...
                FROM services 
                WHERE LOWER(description) LIKE ?
                AND category NOT IN ('LAB', 'LABORATORY', 'RADIOLOGY', 'DIAGNOSTICS')
            """, (f"%{treatment.lower()}%",))
