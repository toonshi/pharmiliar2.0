...
class AIServiceMapper:
    ...
    def _get_related_services(self, services: List[Dict], limit: int = 3) -> List[Dict]:
        """Find related services based on service relationships."""
        if not services:
            return []
            
        # Get codes from input services
        input_codes = set(s.get("code", "") for s in services)
        related_codes = set()
        
        # Find related codes
        for code in input_codes:
            if code in self.service_relationships:
                related_codes.update(self.service_relationships[code])
        
        # Remove input codes from related codes
        related_codes -= input_codes
        
        # Find services for related codes
        related_services = []
        for code in list(related_codes)[:limit]:
            # Try each category
            for category in ["RADIOLOGY", "GENERAL"]:
                found_services = self.service_mapper.find_services(category, code)
                if found_services:
                    for service in found_services:
                        service["category"] = category
                    related_services.extend(found_services)
                    break  # Found the service, no need to check other categories
                    
        return related_services[:limit]
    ...
    def _get_recommendations(self, services: List[Dict], query_context: str, priority: str = "routine") -> str:
        ...
        related = self._get_related_services(services)
